...
`tools/Build-RaceCompatibleStarwind.ps1` gives those Starwind records new `SW_` IDs,
updates their NPC assignments in both the core and remaster patch, and updates the
patch's core-master byte count. It preserves all vanilla races and keeps the Starwind
races selectable.
...
3. Run `tools/Build-RaceCompatibleStarwind.ps1` to build matching core and patch files.
